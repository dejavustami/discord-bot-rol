...
async def katıl(ctx):
    if ctx.author.voice:
        ...
        # Yeniden bağlan ve seste kalması için bekleme ekle
        vc = await channel.connect(self_deaf=True, self_mute=True)
        await ctx.send(f"🎤 **{channel.name}** kanalında 7/24 nöbetteyim!")
        
        # Botun sesten düşmemesi için boş bir döngüde tut (Opsiyonel ama stabilite sağlar)
...
async def on_message(message):
    ...
    if not message.author.guild_permissions.administrator:
        ...
        if len(user_message_times[uid]) > 4:
            ...
            return await message.channel.send(f"⚠️ {message.author.mention}, spam yaptığın için mesajların temizlendi!", delete_after=5)
